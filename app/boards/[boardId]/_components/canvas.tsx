...
import {
  ...
  useStorage,
  useCreateThread,
  useThreads // Import the useCreateThread hook
...
export const Canvas = ({ boardId }: CanvasProps) => {
  ...
  const createThread = useCreateThread(); // Initialize the useCreateThread hook
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (canvasState.mode === CanvasMode.Pencil) {
        setIsZoomEnabled(false); // Disable zoom
  ...
  const handlePointerUp = useMutation(
    ({}, e) => {
      ...
      if (
        ...
        insertPath();
        setIsZoomEnabled(true); // Re-enable zoom
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      switch (e.key) {
        ...
        case "c": {
          if (e.ctrlKey || e.metaKey) {
            const point = { x: camera.x, y: camera.y }; // Use the current camera position as the point
            createThread({  body: {
              version: 1,
              content: []
            }, metadata: {}  }); // Create a new thread at the current point
            break;
          }
        }
      }
